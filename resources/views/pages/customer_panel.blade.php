   ...
   <main>
    ...
    <section class="container-fluid bg_white">
        <ul class="nav nav-pills">
          <li class="nav-tab"><a class="nav-link active" data-toggle="tab" href="#home">لیست سفارشات</a></li>
          
          <li class="nav-tab"><a class="nav-link" data-toggle="tab" href="#menu1">درخواست واریز</a></li>
          <li class="nav-tab"><a  class="nav-link" data-toggle="tab" href="#menu2">تغییر رمز عبور</a></li>
          
        </ul>
        
        <div class="tab-content customer_panel p-4">
          <div id="home" class="customer_order tab-pane  active table-responsive">
              
            <table class="table-hover table-responsive datatable text-center table-bordered">
            <thead>
                <tr class="bg-primary text-white">
                    <th>شماره فاکتور</th>
                    <th class="text-center">نام هتل</th>
                    <th class="text-center">تعداد مسافران</th>
                    <th class="text-center">هزینه کل سفارش- تومان</th>
            ...
            <tbody>
                ...
                @endforeach    
                    ...
                    <td>{{$order->order_trackingCode}}</td>
                    <td>
                        @foreach($all_hotels as $hotel)
                            @if($hotel->hotel_id ==$order->hotel_id)
                                {{$hotel->hotel_comision * $i}}
                   ...
                   {{--@if($order->order_confirm==0) <a class="btn btn-success" href="{{URL::to('/customer/order/edit/'.$order->order_id)}}" >ویرایش سفارش</a>@endif--}}
                        
                      
                            @if($order->order_status != 4)   
                             @if($order->order_confirm ==0)
                                 <a  class="btn btn-danger" href="{{URL::to('/customer/request/'.$order->order_id.'/all')}}">لغو سفارش</a>
                             @endif
                            @else
                              <p>سفارش لغو شد.</p>
                            @endif
                            ...
                            @foreach($all_requests as $request)
                                @if($request->order_id==$order->order_id)
                                    @if($request->customer_id == session::get('customer_id'))
                                        @php($checked=1)
                                    @endif
                                @endif
                            @endforeach
                        ...
                        @if($order->order_status==3 )
                            @if($checked !=0)
                                درخواست تسویه ارسال شده است
                            @else
                             <a class="btn btn-success" href="{{URL::to('/customer/request/'.$order->order_id)}}">درخواست تسویه</a>
                             @endif
                        @endif
                        @endif
                     ...
                     <tr>
                        ...
                        <td>
                            @foreach($all_rooms as $room)
                                @if($room->room_id == $traveler->room_id)
                                    {{$room->room_name}}
                                @endif
                            @endforeach
                        </td>

                          <td>{{$traveler->traveler_des}}</td>
                         <td>
          ...
          <div id="menu1" class="tab-pane fade">
              <div class="row">
                  ...
                  <div class="col-sm-6">
                       <form>
                           <div class="col-9">
                               <input type="text" placeholder="لطفا مبلغ درخواست را به صورت تومان وارد نمایید" class="form-control" value="" name="total_price" >
                           </div>
                           <div  class="col-3">
                 ...
                 <table class="table  table-hover datatable text-center table-bordered ">
                     ...
                     @foreach($all_requests as $request )
                         <tr>
                             ...
                             <td>@if($request->request_pay == 1){{$request->request_pay_date}} @else - @endif</td>
            ...
            <form role="form" class=" form-group col-12"  method="POST" action="{{URL::to('/updatePass')}}">
                           ...
                           <input type="hidden" name="type" value="customer">
                            <div class="form-group ">
                                ...
                                <input type="email" class="form-control" id="username" placeholder="نام کاربری خود را وارد نمایید" name="customer_email" value="{{$customer}}" readonly>
                            </div>
                            <div class="form-group">
