...
function template_functions(){
	...
	$('.datatable').dataTable({
			...
			"oLanguage": {
			"sLengthMenu": "_MENU_ ردیف در هر صفحه"
			}
		} );
...
function add_hotel(option)
{
    var n = ((($('#add_room tr').length)-1)/2) + 1;
	var j=1;
    $("#add_room").append('<tr><td><input type="text" id="room_name" name="room_name['+n+']" required></td><td><select data-rel="chosen" name="room_condition['+n+']" required><option value="0">امکانات اتاق</option>'+option+'</select></td><td><input type="number" name="adult_capacity['+n+']" required ></td><td><input type="number" name="child_capacity['+n+']" required ></td><td><input type="number" name="room_number['+n+']" required></td><td><a  href="#demo'+n+'" data-toggle="collapse">قیمتهای اتاق</a></td><td><textarea class="input-xlarge" id="room_cancel" name="room_cancel['+n+']"  required></textarea></td><td><input type="checkbox" class="cleditor" id="publication_status" name="publication_status['+n+']" checked /></td></tr> <tr> <td colspan="8"><div id="demo'+n+'" class="col-xs-12 collapse show"> <div class="col-xs-12"> <img  onclick="add_price('+n+')" src="http://localhost:8000/backend/img/add.png" class="img_add" title="افزودن قیمت"> <img  onclick="remove_price('+n+')" src="http://localhost:8000/backend/img/minus.png" class="img_add" title="حذف قیمت"> </div> <div class="row row_price"><div class="col-xs-4"><label>از تاریخ</label><input type="text" class="datepicker"  name="room_fromDate['+n+']['+j+']" /></div> <div class="col-xs-4"><label>تا تاریخ</label><input type="text" class="datepicker"  name="room_toDate['+n+']['+j+']"  /></div> <div class="col-xs-4"><label>قیمت</label><input type="number"  name="room_price['+n+']['+j+']" /></div> </div></div></td></tr>');
}
function remove_hotel(event)
{
	if($( '#add_room tr' ).length>2){
		$("#add_room tr:last").remove();
	}else {
		alert('حداقل باید یک اتاق را وارد نمایید')
	}
	$('.datepicker').persianDatepicker();
}

function add_price(room_id){

	var n=$('#demo'+room_id).find('.row_price').length+1;
	var i=Math.floor($('#demo'+room_id).closest('tr').index()/2)+1;
	$('#demo'+room_id).append('<div class="row row_price"><div class="col-xs-4"><label>از تاریخ</label><input type="text" class="datepicker"  name="room_fromDate['+i+']['+n+']" required /></div><div class="col-xs-4"><label>تا تاریخ</label><input type="text" class="datepicker"  name="room_toDate['+i+']['+n+']" required /></div><div class="col-xs-4"><label>قیمت</label><input type="number"  name="room_price['+i+']['+n+']" required/></div></div>');
	$('.datepicker').persianDatepicker();



}
function remove_price(room_id)
{
	if($('#demo'+room_id).find('.row_price').length>1){
		$('#demo'+room_id+" .row_price:last").remove();
	}else {
		alert('حداقل باید یک هزینه را وارد نمایید')
	}
...
function  add_gallery() {
	...
	var n = ($('#gallery').find('input[type="file"]').length) + 2;
	$("#gallery").prepend('<input type="file" name="hotel_gallery['+n+']" id="hotel_image" required><input type="text" name="alt['+n+']" required placeholder="نام تصویر را وارد نمایید *"> <br>');
}

function remove_gallery(event) {
	if($( '.uploader' ).length>2)
	{
		$('.uploader:last').next('.hotel_img').remove();
		$('.uploader:last').next('input[type="text"]').remove();
		$('.uploader:last').remove();
	}else {
		alert('شما باید حداقل یک تصویر برای گالری انتخاب کنید')
	}
}

function  add_gallery_tour() {
    
	var option=new Array();
	var n = ($('#gallery').find('input[type="file"]').length) + 1;
	$("#gallery").prepend('<input type="file" name="tour_gallery['+n+']" id="tour_image" required><input type="text" name="alt['+n+']" required placeholder="نام تصویر را وارد نمایید *"> <br>');
}

function validate() {


	$('#myform').find('input').removeClass("nValid");
	$('#myform').find('.message').remove();


	var inputs = $('#myform').find('input');
	var selects= $('#myform').find('select');
    ...
    for (i = 0; i < inputs.length; i++)
    {
		...
		if (input.prop('required') && input.val() == '') {
            ...
            input.addClass("nValid");
            input.parent().find('label').append('<span class="message">این فیلد اجباری است .</span>');
			if(input.attr('type')=='file'){
				input.parent().before('<p class="message" style="color:red">این فیلد اجباری است .</p> ');
			}
			allRequired=false;
		}
	}
    console.log('INPUTS: '+allRequired);
	...
	for (i = 0; i < selects.length; i++){
		...
		if (select.prop('required') && val== '0') {

			select.addClass("nValid");

			select.parent().find('label').append('<span class="message">این فیلد اجباری است .</span> ');
			if((select.attr('name')=='category_parent2')||(select.attr('name')=='category_parent1'))
			{
				select.parent().parent().find('label').append('<span class="message">این فیلد اجباری است .</span> ');
			}
			allRequired=false;

		}};

	console.log('SELECTS: '+allRequired);

	var wentTime=$('#tour_wenTime').val();
	var backTime=$('#tour_backTime').val();
	var startShow=$('#tour_startShow').val();
	var endShow=$('#tour_endShow').val();
	if(new Date(wentTime) > new Date(backTime)){
		$('#tour_wenTime').css('background-color','rgba(255,0,0,0.1)');
		$('#tour_backTime').css('background-color','rgba(255,0,0,0.1)');
		$('#tour_wenTime').parent().parent().append('<p class="message">تاریخ پایان نمیتواند قبل از تاریخ شروع باشد</p>')
		allRequired=false;
	}else {

		$('#tour_wenTime').css('background-color','rgba(255,255,255,1)');
		$('#tour_backTime').css('background-color','rgba(255,255,255,1)');

	}

	console.log('BACKtIME: '+allRequired);

	if(new Date(startShow) > new Date(endShow)){
		$('#tour_startShow').css('background-color','rgba(255,0,0,0.1)');
		$('#tour_endShow').css('background-color','rgba(255,0,0,0.1)');
		$('#tour_startShow').parent().parent().append('<p class="message">تاریخ پایان نمایش نمیتواند قبل از تاریخ شروع نمایش باشد.</p>')
		allRequired=false;
	}else{

		$('#tour_startShow').css('background-color','rgba(255,255,255,1)');
		$('#tour_endShow').css('background-color','rgba(255,255,255,1)');

	}
	console.log('SHOWING: '+allRequired);


	if ($('#rules').is(':checked')){
		// everything's fine...
	} else {
		$('#rules').parent().parent().parent().append('<span class="message">لطفا قوانین و مقررات را مطالعه نموده ، سپس این گزینه را انتخاب کنید. جهت مطالعه قوانین سایت روی متن پذیرش قوانین کلیک کنید.</span>');
		$('#rules').parent().parent().parent().css('background-color','rgba(255,0,0,0.6)');
		allRequired=false;
	}
	console.log('RULES: '+allRequired);
	if($('.room_types').length != 0)
	{
	if($('.room_types:checkbox:checked').length == 0){
		$('.room_types').parent().parent().parent().parent().append('<span class="message" style="background: rgba(255,0,0,0.6)">حداقل یکی از موارد را انتخاب کنید</span>');
		allRequired=false;
	}
	}
	console.log('ROOMtYPES: '+allRequired);

	if($('.show_tour:checkbox:checked').length == 0){
		$('.show_tour').parent().parent().parent().parent().append('<span class="message" style="background: rgba(255,0,0,0.6)">حداقل یکی از موارد را انتخاب کنید</span>');
		allRequired=false;
	}


	console.log('SHOW TOUR '+allRequired);

	
	
		if($('#tour_lastSecond:checked').length == 0){
		$('#tour_lastSecond').parent().parent().parent().parent().append('<span class="message" style="background: rgba(255,0,0,0.6)">حداقل یکی از موارد را انتخاب کنید</span>');
		allRequired=false;
	}


	console.log('lastSecond '+allRequired);
		...
		{
			alert('لطفا تمام ورودیها را به درستی وارد نمایید.');
			return false;
		}
...
function validate_hotel() {
	...
	var inputs = $('input');
	var selects= $('select');
	var images=$('#myform').find('input[type=file]');
	console.log(images);
	...
	for (i = 0; i < selects.length; i++){
        ...
        for (i = 0; i < images.length; i++)
        {
            ...
            //   if(file_size>20) {
            //       //2097152
            // 	$('#hotel_image').parent().parent().prepend('<p style="color:red" class="message">اندازه تصاویر نباید بالاتر از 2 مگابایت باشد</p>');
            //     allRequired=false;
            //   }
        }
...
function add_traveler_row(rooms,age,hotel){
    	...
    	$.each(rooms, function( index, value ) {
    	   option.push('<option value= '+value['room_id']+'>'+value['room_name']+'</option>');
    	});
    	var traveler_age=new Array();
    	$.each(age, function( index, value ) {
    		traveler_age.push('<option value= '+value['age_id']+'>'+value['age_name']+'</option>');
    	});
    
    	var n = ($('#add_traveler_row tr').length - 1) + 1;
    	$("#add_traveler_row").append('<tr><td><input type="text" name="traveler_name['+n+']" required ></td><td><input type="text" name="traveler_lname['+n+']" required ></td><td><input type="number" name="traveler_age['+n+']" required ></td><td><input name="traveler_mobile['+n+']" type="text"><td><select class="order_room_type" name="room_type['+n+']">'+option+'</select></td><td><input type="checkbox" name="traveler_submit['+n+']" class="form-control"/></td></tr>');
    

}

function remove_traveler_row(event)
{

	if($( '#add_traveler_row tr' ).length>2){
		$("#add_traveler_row tr:last").remove();
	}else {
		alert('حداقل باید یک مسافر وارد کنید')
	}
